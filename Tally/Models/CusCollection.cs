...
namespace LSSyncApp.Tally.Models
{
    ...
    public class ColTDLMessage
    {
                             ...
                             List<string> SysFormulae = null)
        {
            ...
            {
                Field field = new Field(Fld.Key, Fld.Value);
                Field.Add(field);
            }
        ...
        {
            List<string> TdlFilter = new List<string>();
            filters?.ForEach(filter =>
            {
                TdlFilter.Add(filter.FilterName);
                System.Add(new System(filter.FilterName, filter.FilterFormulae));
            });
            Collection = new Collection(colName: colName, colType: colType, nativeFields: nativeFields, filters: TdlFilter);
